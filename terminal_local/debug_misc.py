...
while True:
    ...
    class Turn:
        ...
        def getGameCards(self, gameDeck, playDeck, discardDeck):
            for card in gameDeck:
                self.gamedeck.append((card.suit, card.val))
            for card in playDeck:
                self.playdeck.append((card.suit, card.val))
            for card in discardDeck:
                self.discarddeck.append((card.suit, card.val))

        def sourcePlayerCards(self):
    ...
    class Player():
        ...
        def getPlayerCards(self):
            for card in self.hand:
                self.handcards.append((card.suit, card.val))
            for card in self.bottomCards:
                self.bmcards.append((card.suit, card.val))
            for card in self.topCards:
                self.topcards.append((card.suit, card.val))
    ...
    def playHand(play_list, playable_index, player, playDeck):
        ...
        while True:
            ...
            play_input = str(playable_index[0])
            ###
            ...
            if not all(temp1) or not all(temp2) or not all(temp3): 
                ###
                print("Incorrect input. Try again. playHand()")
                ###
    ...
    while gameOver == False:
        ...
        """with open("history.txt", "w") as f:
            thisTurn = Turn()
            thisTurn.getGameCards(gameDeck, playDeck, discardDeck)
            for player in playerList:
                player.getPlayerCards
            
            l1 = ["\n", str(len(turn_list)), " : This is a new turn", str(turn)]
            l2 = ("attack", str(attack))
            l3 = ("clockwise", str(clockwise))
            l4 = ("\nGameDeck: ", str(thisTurn.gamedeck))
            l5 = ("\nPlayDeck: ", str(thisTurn.playdeck))
            l6 = ("\nDiscardDeck: ", str(thisTurn.discarddeck))
            l7 = ("\n")
            
            for player in playerList:
                l8 = ("\n", str(player.name))
                l9 = ("Hand : ", str(player.handcards))
                l10 = ("Bottom Cards : ", str(player.bmcards))
                l11 = ("Top Cards : ", str(player.topcards))
        ...
        #midgame/endgame
            ...
            if temp1:
                ...
                ###
                print("These are your face up cards.")
                ###
                ...
                if not playable_index:
                    ...
                    ###
                    print("You cannot play. You eat the pile")
                    ###
                ...
                if player.bottomCards[r].val not in playable_cards:
                    ...
                    ###
                    print("you cannot play this card (facedown cards)", player.name)
                    ###
                    ...
                    ###
                    print("you can play this card (facedown cards)", player.name)
                    ###
        ...
        if len(turn_list) > 1000:
            print("!!!!bots are in a loop. cannot get out!!!!")
            break
